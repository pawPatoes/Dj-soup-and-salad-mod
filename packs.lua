...
SMODS.Booster {
    ...
    create_card = function(self, card, i)
        local rarity_roll = pseudorandom('dj_mega_rarity'..G.GAME.round_resets.ante..i)
        local target_rarity = 1 
        if rarity_roll > 0.96 then target_rarity = 4           
        elseif rarity_roll > 0.80 then target_rarity = 3   
        elseif rarity_roll > 0.30 then target_rarity = 2                    
        ...
        local chosen_key = (#dj_pool == 0) and 'j_joker' or pseudorandom_element(dj_pool, pseudorandom('dj_mega_spawn'..i))
